...
class Policy_net:
    ...
    def __init__(self, name: str, sess, ob_space, policy_n, k=1, activation=tf.nn.relu, units=128, trainable=True):
        ...
        with tf.variable_scope(name):
            ...
            with tf.variable_scope('policy_net'):
                ...
                self.weight = tf.layers.dense(inputs=layer_1, units=policy_n, activation=tf.nn.softmax,
                                              trainable=trainable)
            ...
            self.policy_stochastic = tf.multinomial(tf.log(self.weight), num_samples=1)
            self.policy_stochastic = tf.reshape(self.policy_stochastic, shape=[-1])
    ...
    def act_prob(self, obs):
        ...
        policy, v = self.sess.run([self.policy_stochastic, self.v_preds], feed_dict={self.obs: obs})
...
class PPOTrain:
    ...
    def __init__(self, name, sess, Policy, Old_Policy, policy_n, gamma=0.9, clip_value=0.2, c_1=1., c_2=0.01):
        ...
        with tf.name_scope(name):
            ...
            weight = tf.reduce_sum(tf.multiply(self.Policy.weight, tf.one_hot(self.p_select, policy_n)), axis=1, name='prob')

            # probabilities of actions which agent took with old weight weighted
            weight_old = tf.reduce_sum(tf.multiply(self.Old_Policy.weight, tf.one_hot(self.p_select, policy_n)), axis=1, name='old_prob')

            with tf.variable_scope('loss'):
                with tf.variable_scope('clip_loss'):
                    # construct computation graph for loss_clip
                    ratios = tf.exp(tf.log(tf.clip_by_value(weight, 1e-10, 1)) -
                                    tf.log(tf.clip_by_value(weight_old, 1e-10, 1)), name='ratios')
                ...
                sum_policy_w1 = tf.summary.histogram('policy_weight',
    ...
    def train(self, obs, act_prob, gaes, rewards, v_preds_next):
        summary, _, total_loss = self.sess.run([self.merged, self.train_op, self.total_loss],
                                               feed_dict={self.Policy.obs: obs,
                                                          self.Old_Policy.obs: obs,
                                                          self.p_select: act_prob,
    ...
    def ppo_train(self, observations, p_select, rewards, gaes, v_preds_next, verbose=False):
        if verbose:
            print('PPO train now..........')
        ...
        p_select = p_select[:, np.newaxis]
        gaes = gaes[:, np.newaxis]
        v_preds_next = v_preds_next[:, np.newaxis]

        print(observations.shape)
        print(p_select.shape)
        print(gaes.shape)
        print(rewards.shape)
        print(v_preds_next.shape)

        dataset = np.hstack((observations, p_select, rewards, gaes, v_preds_next))
...
class MixPolicyBA(policy):
    ...
    def save_transition(self, obs, next_obs, p_select, reward, v, done, t):
        """
        ...
        if not done:
            ...
            self.traj_policy.append(p_select)
    ...
    def train(self):
        ...
        with self.sess.as_default():
            with self.graph.as_default():
                ...
                p_select = np.array(self.p_memory).astype(dtype=np.float32)
                ...
                for s in self.PPOTrain.ppo_train(observations, p_select, rewards, gaes, v_preds_next):
...
if __name__ == "__main__":
    ...
    mix_policy1 = MixPolicyBA(ps1_n, log_path='model/mix/bplog1', k=4)
    mix_policy2 = MixPolicyBA(ps2_n, log_path='model/mix/bplog2', k=4)
    ...
    while True:
        ...
        if epoch % _save_num == 0:
            mix_policy1.save_model('model/mix/p1bp/{0}.ckpt'.format(_save_times))
            mix_policy2.save_model('model/mix/p2bp/{0}.ckpt'.format(_save_times))
            epoch += 1
            _save_times += 1
