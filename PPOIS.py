    ...
    def  __init__(self, name: str, sess, ob_space, act_space, k=1, activation=tf.nn.relu, units=128, trainable=True):
        ...
        with tf.variable_scope(name):
            ...
            with tf.variable_scope('policy_net'):
                layer_1 = tf.layers.dense(inputs=self.obs, units=units , activation=activation, trainable=trainable)
...
class PPOTrain:
    ...
    def __init__(self, name, sess, Policy, gamma=0.99, clip_value=0.2, c_1=0.5, c_2=0.05):
        """
        :param Policy:
        :param Old_Policy:
        :param gamma:
        ...
        with tf.name_scope(name):
            ...
            with tf.variable_scope('loss'):
                ...
                clipped_is_ratios = tf.clip_by_value(ratios, clip_value_min=0, clip_value_max=2)
                loss_vf = tf.squared_difference(clipped_is_ratios * (self.rewards + self.gamma * self.v_preds_next), v_preds)
...
class PPOIS(policy):
    ...
    def __init__(self,
        ...
        with self.sess.as_default():
            with self.graph.as_default():
                ...
                self.PPOTrain = PPOTrain('train', self.sess, self.pi)
    ...
    def train(self, observations, actions, gaes, rewards, v_preds_next, mu_prob):
        ...
        with self.sess.as_default():
            with self.graph.as_default():
                ...
                mu_prob = np.hstack(mu_prob).astype(np.float32)
